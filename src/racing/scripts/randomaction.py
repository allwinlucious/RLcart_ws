...
class racecar():
    ...
    def computeReward(self):
        if self.isCrashed():
            self.info = {'crashed after laps', self.state[4]}
            self.done = True
            self.reward += -100
            if self.debug == True:
...
class racetrack_env(Env):
    ...
    def pause(self):
        ...
        try:
            pause = rospy.ServiceProxy('pause', Empty)
            pause()
        except rospy.ServiceException as e:
            print("Service call for pause failed: %s"%e)
            
    def resume(self):
        rospy.wait_for_service('resume')
        try:
            pause = rospy.ServiceProxy('resume', Empty)
            pause()
        except rospy.ServiceException as e:
            print("Service call for resume failed: %s"%e)
